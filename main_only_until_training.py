...
for train_index, test_index in split_data.split(housing, housing['income_cat']):
    strata_train_set = housing.loc[train_index].drop("income_cat", axis=1) # We will work on this data
    strata_test_set = housing.loc[test_index].drop("income_cat", axis=1) # set aside the test data
...
# lin_rmse = root_mean_squared_error(housing_labels, linear_predictions)
...
decisiontree_predictions = dec_reg.predict(housing_prepared)
#dec_rmse = root_mean_squared_error(housing_labels, decisiontree_predictions)
dec_rmses = -cross_val_score(dec_reg, housing_prepared, housing_labels, scoring="neg_root_mean_squared_error", cv=10) # the less the error that much better hence negative
